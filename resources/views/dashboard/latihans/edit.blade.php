    ...
    <form method="post" action="/dashboard/latihans" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="jenislatihan_id">
            ...
            @if (old('jenislatihan_id', $latihan->jenislatihan_id) == $jenislatihan->id)
             <option value="{{ $jenislatihan->id }}" selected>{{ $jenislatihan->name }}</option>
            @else
             <option value="{{ $jenislatihan->id }}">{{ $jenislatihan->name }}</option>
            @endif
            @endforeach
          </select>
        </div>
